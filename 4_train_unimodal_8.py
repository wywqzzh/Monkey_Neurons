...
def expand_dimension(v):
    return np.array([i*np.ones((2,8)) for i in v])
...
def init():
    ...
    RawR_ves = np.array((RawR_ves))
    RawR_ves = [R[0:8, :] for R in RawR_ves]
    RawR_ves = np.array(RawR_ves)

    RawR_cam = np.array((RawR_cam))
    RawR_cam = [R[:, 0:8] for R in RawR_cam]
    RawR_cam = np.array(RawR_cam)
    ...
    S_vest_m_cam= np.zeros((SIZE, 8, 8))
    S_vis_m_cam= np.zeros((SIZE, 8, 8))
    S_vest_m_ves= np.zeros((SIZE, 8, 8))
    S_vis_m_ves= np.zeros((SIZE, 8, 8))
    c_vest = 0
    c_vis = 1
