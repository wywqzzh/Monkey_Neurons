...
def init():
    ...
    selected_neurons = [i for i in range(SIZE) if
                        corr_list[i] > 0.5]  # select good neurons: correlation between trials over 0.4
    ...
    data_write('../resultConsiderCoherence/参数4_nobound_bimodal.xls',(np.array(res.x).reshape(5,len(selected_neurons)).tolist()))
    print('Error:', res.fun)
