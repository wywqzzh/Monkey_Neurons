...
def optimize_f(current_vect):
    ...
    normPool = current_vect[-1]*np.mean(L_neuron**n3)
    alpha = current_vect[1]
    expand_alpha=expand_dimension(alpha)
    for i in range(len(expand_alpha)):
        expand_alpha[i]=expand_alpha[i]+normPool[i]
    # Rmax_m = np.array([m*np.full((9,9),1) for m in current_vect[4]])
    Rs = Rmax_m*(L_neuron**n3)/expand_alpha
...
def init():
    ...
    init_vect = np.array([np.random.rand(SIZE) + 9.5, np.random.rand(SIZE) + 0.5,
                          np.zeros((SIZE,)) + 0.5, np.zeros((SIZE,)) + 0.55,
                          np.zeros((SIZE,))+1.1])  # 参数初始值
    global Rmax_m

    Rmax_m = np.zeros((SIZE, 8, 8))
    ...
    data_write('./result/参数4_nobound_bimodal.xls',(np.array(res.x).reshape(5,len(selected_neurons)).tolist()))
    print('Error:', res.fun)
